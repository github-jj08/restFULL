...
public class TravelCourseController {
   ...
   public List<GoodsVO> getGoods(HttpServletRequest req) {
	   log.info("addTravelCourse");
	   String[] myGoods = req.getParameterValues("myGoods[]");
	   log.info(myGoods.length);
	   for(int i=0;i<myGoods.length;i++) {
		   log.info("상품 번호 목록 ? " + myGoods[i]);
	   }
	   List<GoodsVO> goodslist = adboardService.myGoods(myGoods);
	   return goodslist;
   }
